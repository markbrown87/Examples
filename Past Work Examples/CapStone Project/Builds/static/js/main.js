$(document).ready(function() {
  ...
  var criteriaDict = JSON.parse(document.getElementById("myVar").value);
  //console.log(Object.values(criteriaDict))
  var criteria = Object.values(criteriaDict);
  for (i = 0; i < criteria.length; i++) {
    criteria[i] = criteria[i].replace(/\+/g, " ")
  }
  var nb_sliders = criteria.length; // nb of range sliders
  ...
  function init() {
    ...
    for (i = 0; i < nb_sliders; i++) {
      ...
      tr.append('td')
        ...
        .attr('step', 1)
        .attr('min', 1)
        .attr('max', (101 - nb_sliders));
      tr.append('td')
        .attr('class', 'range_value');

      $("#weight_" + i).val(parseInt(json[i], 10))
    }
  ...
  function create_post() {
    ...
    for (i = 0; i < result.length; i++) {
      var val = result[i.toString()].value;
      var field = document.getElementById('weight_' + i.toString());
      field.value = val;
    }
  };
});
